 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: core_config.h 2318 2012-02-23 12:22:35Z ertl-honda $
 */
...
typedef struct {
    char *start_rom_label;
    uint32_t atrsz_rom;
    char *start_ram_label;
    uint32_t atrsz_ram;
    char *start_srpw_label;
    uint32_t atrsz_srpw;
    char *start_mem_label;
    uint32_t atrsz_mem;
} DOMINIB_INFO;

typedef struct {
    DOMINIB_INFO *p_dominib_info;
} DOMINICTXB;
 ...
 *  入り口処理でtrueにセットされる
 */
#define t_sense_priv(p_tcb)   ((p_tcb)->tskctxb.priv_mode)
#define i_sense_priv(p_tcb)   ((p_tcb)->tskctxb.priv_mode)
#define t_sense_priv_runtsk() ((p_runtsk)->tskctxb.priv_mode)
#define i_sense_priv_runtsk() ((p_runtsk)->tskctxb.priv_mode)
...
#define PROBE_STACK(sp, size) \
				(ALIGN_TYPE(sp, uint32_t) && probe_stack(sp, size))
#endif /* PROBE_STACK */
...
#define USRTEX_STKSZ ((8+1)*4)

/*
 *  SVCのパラメータ
 */
#define NO_RET_INT      0   /* ret_int_r時のsvcの引数 */
#define NO_UTASK_START  1   /* ユーザタスク起動時のsvcの引数 */
#define NO_SVC          2   /* サービスコール呼び出し時のsvcの引数 */
#define NO_SVC_R        3   /* サービスコールリターン時のsvcの引数 */
#define NO_UTEX         4   /* prepare_texrtn_utaskのsvcの引数 */
#define NO_RET_TEX      5   /* ret_tex呼び出し時のsvcの引数 */
#define NO_RET_TEX_R    6   /* ret_tex_r呼び出し時のsvcの引数 */
